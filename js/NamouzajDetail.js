...
fetch("../data/files.json")
  ...
  .then((data) => {
    ...
    document.getElementById("pic").appendChild(imageElement.firstChild);
     ...
     scannedButton.onclick = () => downloadFile(scannedUrl, `${question.name}.${fileType}`);
     // Update HTML to display scanned download button
document.getElementById("scannedUrl").innerHTML = `
<button class="btns" style="margin-top: 3rem;" onclick="downloadFile('${scannedUrl}', '${question.name}.${fileType}')">Download ${fileType.toUpperCase()} File</button>
`;
